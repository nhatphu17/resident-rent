...
export function useGeolocation() {
  ...
  const getLocation = (attempt: number = 0) => {
    ...
    console.log(`[Geolocation] Attempt ${attempt + 1}/${maxRetries + 1} - Options:`, options);
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (
          ...
          console.log('[Geolocation] Invalid coordinates received');
      ...
      (error) => {
        ...
        if (attempt < maxRetries && error.code !== error.PERMISSION_DENIED) {
          ...
          console.log(`[Geolocation] Retrying, attempt ${attempt + 1}/${maxRetries}`);
          setTimeout(() => {
            getLocation(attempt + 1);
          }, 2000 * (attempt + 1)); // Exponential backoff: 2s, 4s
        } else {
          console.log('[Geolocation] Failed after all retries');
          setState({
            latitude: null,
            longitude: null,
            error: errorMessage,
  ...
  const retry = () => {
    console.log('[Geolocation] Manual retry triggered');
